...
from mesil.data.metadata import get_analysis, get_fluorescence_type, get_synthesis_code
from mesil.plotting.customize import FLS_xlim
from mesil.plotting.plotters import custom_xy_plotter


def main(data_folder: Path = Settings().paths.data.processed, plot_root = Settings().paths.figures):
    data_to_plot = [datafile for datafile in filter_extensions(data_folder, extensions=[".csv"])]
    ...
    for datafile in data_to_plot:
        ...
        output_path = plot_root / get_synthesis_code(datafile) / f"{analysis}-{datafile.stem}.png"
